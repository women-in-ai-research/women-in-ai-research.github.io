...
function BlogPost({ href, title, excerpt, titleGradient, textColor }: BlogPostProps) {
    return (
        <a
            href={getPageUrl(href)}
            className="block hover:bg-white/5 p-4 rounded-lg transition-colors duration-300"
        >
            <h4 className={`text-transparent bg-clip-text bg-gradient-to-r ${titleGradient} font-semibold mb-2`}>
                {title}
            </h4>
